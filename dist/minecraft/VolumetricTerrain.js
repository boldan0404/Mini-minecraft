...
(function (BlockType) {
    BlockType[BlockType["AIR"] = -1] = "AIR";
    BlockType[BlockType["GRASS"] = 0] = "GRASS";
    BlockType[BlockType["STONE"] = 1] = "STONE";
    BlockType[BlockType["WATER"] = 2] = "WATER";
    BlockType[BlockType["SNOW"] = 3] = "SNOW";
    BlockType[BlockType["WOOD"] = 4] = "WOOD";
    BlockType[BlockType["LEAVES"] = 5] = "LEAVES";
    BlockType[BlockType["DIRT"] = 6] = "DIRT";
    BlockType[BlockType["SAND"] = 7] = "SAND";
    BlockType[BlockType["COAL"] = 8] = "COAL";
    BlockType[BlockType["IRON"] = 9] = "IRON";
    BlockType[BlockType["GOLD"] = 10] = "GOLD";
    BlockType[BlockType["DIAMOND"] = 11] = "DIAMOND";
    BlockType[BlockType["LAVA"] = 12] = "LAVA";
})(BlockType || (BlockType = {}));
/**
...
export class VolumetricTerrain {
     ...
     */
    constructor(chunkX, chunkZ, size) {
        // Noise configuration parameters - reduced values for better performance
        this.CAVE_SCALE = 18.0;
        this.CAVE_THRESHOLD = 0.42;
        this.SURFACE_VARIATION_SCALE = 12.0;
        this.SURFACE_DETAIL_SCALE = 24.0;
        this.ORE_SCALE = 8.0;
        this.BIOME_SCALE = 80.0;
        this.chunkX = chunkX;
        this.chunkZ = chunkZ;
        ...
        this.blockData = Array(size).fill(null).map(() => Array(size).fill(null).map(() => Array(size).fill(null)));
     ...
     */
    applyBiomeHeightVariation() {
        // Generate biome map using large-scale noise
        const biomeNoise = this.generate2DNoise(this.size, this.size, this.BIOME_SCALE, `${this.seed}-biome`, 1);
        for (let x = 0; x < this.size; x++) {
            for (let z = 0; z < this.size; z++) {
    ...
    generateVolumetricBlocks() {
        ...
        const densityNoise = this.generate3DNoise(this.size, this.size, this.size, this.SURFACE_DETAIL_SCALE, `${this.seed}-density`, 2);
    ...
    generateCaves() {
        ...
        const caveNoise = this.generate3DNoise(this.size, this.size, this.size, this.CAVE_SCALE, `${this.seed}-caves`, 2 // Reduced octaves for better performance
        );
        // Secondary noise for more interesting cave shapes
        const detailNoise = this.generate3DNoise(this.size, this.size, this.size, this.CAVE_SCALE / 2, `${this.seed}-cave-detail`, 1 // Reduced octaves
        );
        const worldY = this.chunkY;
        ...
        for (let x = 0; x < this.size; x++) {
            for (let y = 0; y < this.size; y++) {
                for (let z = 0; z < this.size; z++) {
                    ...
                    if (worldHeight > this.heightMap[x][z] - 5)
                        continue;
                    // Skip caves too close to water level to prevent flooding
                    if (worldHeight < 37 && worldHeight > 25)
                        continue;
                    // Get the noise value
        ...
        for (let x = 0; x < this.size; x++) {
            for (let y = 0; y < this.size; y++) {
                for (let z = 0; z < this.size; z++) {
                    ...
                    if (this.blockData[x][y][z] !== BlockType.STONE)
                        continue;
     ...
     */
    generate2DNoise(width, height, scale, seed, octaves = 1) {
     ...
     */
    generate3DNoise(width, height, depth, scale, seed, octaves = 1) {
        ...
        const result = Array(adjustedWidth).fill(0).map(() => Array(adjustedHeight).fill(0).map(() => Array(adjustedDepth).fill(0)));
        const persistence = 0.5;
        for (let octave = 0; octave < octaves; octave++) {
            const octaveScale = scale / Math.pow(2, octave);
            const amplitude = Math.pow(persistence, octave);
     ...
     */
    interpolate3DToSize(grid, targetWidth, targetHeight, targetDepth) {
        const sourceWidth = grid.length;
        const sourceHeight = grid[0].length;
        const sourceDepth = grid[0][0].length;
        const result = Array(targetWidth).fill(0).map(() => Array(targetHeight).fill(0).map(() => Array(targetDepth).fill(0)));
     ...
     */
    trilinearInterpolation(grid, x, y, z) {
        const x1 = Math.floor(x);
        const x2 = Math.min(x1 + 1, grid.length - 1);
    ...
    smoothHeightMap() {
        ...
        const smoothedHeights = Array(this.size).fill(0).map((_, i) => Array(this.size).fill(0).map((_, j) => this.heightMap[i][j]));
...
//# sourceMappingURL=VolumetricTerrain.js.map