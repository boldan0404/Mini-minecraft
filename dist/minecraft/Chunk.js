 ...
 */
    ...
    generateTrees() {
        ...
        for (let i = 0; i < numTrees; i++) {
            ...
            if (this.isTerrainSteep(localX, localZ))
                continue;
            // Convert to world coordinates
            const worldX = Math.floor(this.x - this.size / 2) + localX;
            const worldZ = Math.floor(this.z - this.size / 2) + localZ;
...
class FractalTree {
    ...
    generatePythagorasTree(rng) {
        ...
        const buildBranch = (x, y, z, length, angle, dir, depth) => {
            if (depth <= 0 || length < 0.5)
                return;
            ...
            // Draw the branch
            this.drawLine(Math.round(x), Math.round(y), Math.round(z), Math.round(x + dx), Math.round(y + dy), Math.round(z), depth > 2 ? 5 : 4 // Use leaves for outer branches
            );
    ...
    generateKochCurve(rng) {
        ...
        const buildKochBranch = (x, y, z, length, dir, depth) => {
            if (depth <= 0 || length < 0.5)
                return;
            // Calculate endpoint
            const [dx, dy, dz] = dir;
            ...
            // Draw the branch
            this.drawLine(Math.round(x), Math.round(y), Math.round(z), Math.round(newX), Math.round(newY), Math.round(newZ), depth > 1 ? 5 : 4 // Use leaves for outer branches
            );
            if (depth > 0) {
    ...
    generateSierpinskiTriangle(rng) {
        const iterations = 4;
        const sideLength = 8;
        ...
        const drawTriangle = (x, y, z, size, blockType) => {
            ...
            this.drawLine(Math.round(points[0][0]), Math.round(points[0][1]), Math.round(points[0][2]), Math.round(points[1][0]), Math.round(points[1][1]), Math.round(points[1][2]), blockType);
            this.drawLine(Math.round(points[1][0]), Math.round(points[1][1]), Math.round(points[1][2]), Math.round(points[2][0]), Math.round(points[2][1]), Math.round(points[2][2]), blockType);
            this.drawLine(Math.round(points[2][0]), Math.round(points[2][1]), Math.round(points[2][2]), Math.round(points[0][0]), Math.round(points[0][1]), Math.round(points[0][2]), blockType);
            return points;
        };
        // Recursively subdivide the triangle
        const subdivideTriangle = (points, depth) => {
            if (depth <= 0)
                return;
            // Draw this triangle
            const [top, bottomLeft, bottomRight] = points;
            const blockType = depth > iterations / 2 ? 4 : 5; // Use leaves for outer triangles
            this.drawLine(Math.round(top[0]), Math.round(top[1]), Math.round(top[2]), Math.round(bottomLeft[0]), Math.round(bottomLeft[1]), Math.round(bottomLeft[2]), blockType);
            this.drawLine(Math.round(bottomLeft[0]), Math.round(bottomLeft[1]), Math.round(bottomLeft[2]), Math.round(bottomRight[0]), Math.round(bottomRight[1]), Math.round(bottomRight[2]), blockType);
            this.drawLine(Math.round(bottomRight[0]), Math.round(bottomRight[1]), Math.round(bottomRight[2]), Math.round(top[0]), Math.round(top[1]), Math.round(top[2]), blockType);
            // Calculate midpoints
            const midTop = [
    ...
    generateDragonCurve(rng) {
        ...
        for (let i = 0; i < dragonString.length; i++) {
            ...
            if (char === 'F') {
                ...
                this.drawLine(Math.round(x), Math.round(y), Math.round(z), Math.round(newX), Math.round(newY), Math.round(newZ), blockType);
                x = newX;
                y = newY;
                z = newZ;
            }
    ...
    generateBranchingFractal(rng) {
        ...
        const buildBranch = (x, y, z, length, angleHorizontal, angleVertical, depth) => {
            if (depth <= 0 || length < 0.5)
                return;
            ...
            for (let i = 0; i < branchFactor; i++) {
                ...
                buildBranch(newX, newY, newZ, length * (0.6 + 0.1 * i), // Slightly different lengths
                newHAngle, newVAngle, depth - 1);
            }
        };
    ...
    drawLine(x1, y1, z1, x2, y2, z2, blockType) {
    ...
    addBlock(x, y, z, type) {
        ...
        const exists = this.blocks.some(block => block.x === x && block.y === y && block.z === z);
        if (!exists) {
