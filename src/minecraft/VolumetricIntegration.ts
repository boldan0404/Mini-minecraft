 ...
 */
export function createVolumetricMinecraft(canvas: HTMLCanvasElement): MinecraftAnimation {
 ...
 */
function replaceChunks(minecraft: MinecraftAnimation): void {
    try {
        // Access protected properties (using 'any' type to bypass TypeScript protection)
        const mc = minecraft as any;
 ...
 */
function updateRenderSystem(minecraft: MinecraftAnimation): void {
    try {
 ...
 */
function setupRenderPass(renderPass: RenderPass, minecraft: MinecraftAnimation): void {
    ...
    renderPass.addInstancedAttribute("aBlockType",
        1,
        gl.FLOAT,
        false,
    ...
    renderPass.addUniform("uTimeOfDay",
        (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
 ...
 */
function patchInteractionMethods(minecraft: MinecraftAnimation): void {
    try {
        const mc = minecraft as any;
        ...
        mc.interactWithBlock = function(isRemove: boolean = false): boolean {
